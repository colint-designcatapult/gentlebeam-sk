...
{
    internal class DummyChildEntryCommandsTests
    {
        const long PARENT_ID = 1;
        ...
        internal class TestDummyCommands : DummyChildEntryCommands<TestDataClass, TestDataClass>
        {
            public TestDummyCommands() : base(entry => entry.ParentId)
            {
            }
        }
        ...
        {
            var emptyList = commands.ReadListAsync(PARENT_ID).GetAwaiter().GetResult();
            Assert.That(emptyList, Is.Not.Null);
            Assert.That(emptyList, Is.Not.Empty);
        }

        [Test]
        public void ReadListAsync_NoDataTest()
        {
            var emptyList = commands.ReadListAsync(PARENT_ID + 1).GetAwaiter().GetResult();
            Assert.That(emptyList, Is.Not.Null);
        ...
        [Test]
        public void CreateAsync_WithNewParent_Test()
        {
            var newDataEntry = commands.CreateAsync(new TestDataClass { ParentId = PARENT_ID + 1}).GetAwaiter().GetResult();
            Assert.That(newDataEntry, Is.Not.Null);
            Assert.That(newDataEntry.Id, Is.Not.EqualTo(dataEntry.Id)); // should be unique
        }
        ...
        public void DeleteAsync_ForExistentObject_Test()
        {
            ...
            var parentList = commands.ReadListAsync(dataEntry.ParentId).GetAwaiter().GetResult();
