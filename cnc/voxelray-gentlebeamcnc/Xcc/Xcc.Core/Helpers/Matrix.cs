...
namespace Xcc.Core.Helpers
{
    public class Matrix
    {
        ...
        public bool Equals(Matrix obj, double precision = 0.000001)
        {
            if (obj == null)
                return false;
            ...
            for (int col = 0; col < cols; col++)
                if (Math.Abs(this[row, col] - obj[row, col]) > precision)
                    return false;
        ...
        public Matrix SolveWith(Matrix v)                        // Function solves Ax = v in confirmity with solution vector "v"
        {
            ...
            // L, U, pi is not null here, null-forgiving operator is used
        ...
        public double Det()                         // Function for determinant
        {
            ...
            double det = detOfP;
            for (int i = 0; i < rows; i++) det *= U![i, i]; // U is not null here, null-forgiving operator is used
            return det;
        }
        ...
        public Matrix GetP()                        // Function returns permutation matrix "P" due to permutation vector "pi"
        {
            ...
            for (int i = 0; i < rows; i++) matrix[pi![i], i] = 1; // pi is not null here, null-forgiving operator is used
            return matrix;
        }
        ...
        public static Matrix Parse(string ps)                        // Function parses the matrix from string
        {
            ...
            { 
                throw new MException("Wrong input format!", formatException); 
            }
    ...
    // Some util nested matrix classes:
    public class Matrix2x3 : Matrix
    {
        public Matrix2x3() : base(2, 3)
        { }
    }
    public class Vector3 : Matrix
    {
        public Vector3() : base(3, 1)
        { }
    }
